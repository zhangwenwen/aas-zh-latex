...
以前的模板假装在使用\LaTeX 便利性进行论文排版，存在如下问题：
\begin{enumerate}
	\item 首页结束位置需要根据实际的内容添加 one column or two columns进行手动微调，\textbf{这里我使用的解决办法是简单粗暴，把所有的分栏的都去掉，直接使用一个balance指令进行最后一页内容的对齐；}
	\item 图表不能进行标号引用，手动调节，以前不使用的原因是因为，在手动双栏的时候，figure等添加的图像不显示，\textbf{在上条取消了two column就自然不存在问题了；}
	\item 公式不能通栏，\textbf{直接放在一个可以通栏的figure* 里面；}
	\item 显示的作者信息不是仿宋体，\textbf{直接使用ctex包进行替换；}
\end{enumerate}

还需要进行的是改造 参考文献部分，因为自动化学报使用的是中英文双语的参考文献，需要比较多的工作量，接下来方便的时候进行。
...
\caption{An exampletable in one column}
